...
function initSliders() {
  if (document.querySelector(".page-product__slider")) {
    const parentSliders = document.querySelector(".page-product__slider").parentElement;
    ...
    const sliderBullets = parentSliders.querySelector(".swiper-pagination");
    const pageProductThumbs = new Swiper(".page-product__thumb", {
    ...
    new Swiper(".page-product__slider", {
      // Вказуємо склас потрібного слайдера
      // Підключаємо модулі слайдера
      // для конкретного випадку
...
function addActiveParentClass(buttonsClass, activeClass, clickNotInside = true) {
  ...
  document.addEventListener("click", function(e) {
    const isClickInside = Array.from(buttons).some((button) => {
      ...
      return button.contains(e.target) || parent.contains(e.target);
    });
    if (!isClickInside && clickNotInside) {
...
addActiveParentClass(".btn-toggle", "is-open");
function catalogInit() {
  const htmlElement = document.documentElement;
  const catalogBlock = document.querySelector(".catalog-block");
  if (!catalogBlock) return;
  const catalogBtnToggle = catalogBlock.querySelector(".catalog-block__btn");
  const catalogBtnClose = catalogBlock.querySelector(".catalog-block__close");
  const catalogDropdown = catalogBlock.querySelector(".catalog-block__dropdown");
  const catalogList = catalogBlock.querySelector(".catalog-block__list");
  const catalogSubcatalog = catalogBlock.querySelectorAll(".subcatalog");
  function openCatalog() {
    htmlElement.classList.toggle("catalog-open");
    const catalogBlockRect = catalogBlock.getBoundingClientRect();
    const catalogListHeight = catalogList.offsetHeight;
    catalogDropdown.style.height = `${window.innerHeight - catalogBlockRect.bottom}px`;
    catalogSubcatalog.forEach((item) => {
      item.style.height = `${catalogListHeight}px`;
    });
  }
  function closeCatalog() {
    htmlElement.classList.remove("catalog-open");
  }
  catalogBtnToggle.addEventListener("click", function(e) {
    e.stopPropagation();
    openCatalog();
  });
  if (typeof isMobile !== "undefined" && typeof isMobile.any === "function" && !isMobile.any()) {
    catalogBtnToggle.addEventListener("mouseenter", function(e) {
      e.stopPropagation();
      if (!htmlElement.classList.contains("catalog-open")) {
        openCatalog();
      }
    });
  }
...
document.querySelector(".catalog-block__btn") ? window.addEventListener("load", catalogInit) : null;
...
addActiveHoverParentClass(".hover-toggle", "is-open", ".subcatalog");
const starGroups = document.querySelectorAll(".options-stars");
if (starGroups.length) {
  starGroups.forEach((group) => {
    const items = group.querySelectorAll(".options-stars__item");
    items.forEach((item, index) => {
      const input = item.querySelector(".options-stars__input");
